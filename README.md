...
程序会自动在 FLAC 文件所在目录的同级创建 `mp3` 文件夹，所有转换的 MP3 文件都会保存到这个文件夹中。

**示例：**
- 选择文件：`/Users/Music/FLAC/song.flac` → MP3 输出到：`/Users/Music/mp3/song.mp3`
- 选择文件夹：`/Users/Music/FLAC/` → MP3 输出到：`/Users/Music/mp3/`

**规则：**
- 如果选择的是文件，在文件父目录的同级创建 `mp3` 文件夹
- 如果选择的是文件夹，在文件夹的同级创建 `mp3` 文件夹
- 如果 `mp3` 文件夹已存在，直接使用
- 如果 `mp3` 文件夹不存在，自动创建
...
2. **自定义输出目录规则**
   - 修改 `ui.py` 中的 `_get_mp3_output_dir()` 方法
   - 可以改为输出到任意自定义位置
